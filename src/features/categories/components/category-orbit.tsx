...
function orbStyle(color: string, dominant: boolean) {
  return {
    background: `
      radial-gradient(circle at 34% 26%, rgba(255,255,255,0.38), transparent 52%),
      radial-gradient(circle at 50% 55%, color-mix(in srgb, ${color} ${dominant ? 66 : 52}%, rgba(16,14,36,0.30)) 62%, color-mix(in srgb, ${color} 28%, rgba(10,9,26,0.45)))
    `,
    border: `1.5px solid color-mix(in srgb, ${color} ${dominant ? 82 : 66}%, transparent)`,
    boxShadow: `
      inset 0 1px 2px rgba(255,255,255,0.32),
      inset 0 -8px 18px rgba(0,0,0,0.26),
      0 0 ${dominant ? 46 : 28}px color-mix(in srgb, ${color} ${dominant ? 52 : 38}%, transparent),
...
export function CategoryOrbit({ categories, currency }: CategoryOrbitProps) {
  ...
  const satellites = arrangement.slice(1, 13).map((id) => byId.get(id)).filter(Boolean) as CategorySummary[]
  ...
  return (
    <div
      ref={containerRef}
      className="ambient-haze relative mx-auto mb-6 w-full"
      style={
        {
          aspectRatio: ASPECT,
    ...
    >
      <motion.div className="absolute inset-0" style={{ x: parallaxX, y: parallaxY }}>
        ...
        <div
          aria-hidden="true"
          className="absolute left-1/2 aspect-square w-[80%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/8"
          style={{
            top: `${CENTER_Y * 100}%`,
            boxShadow: 'inset 0 0 40px rgba(124,60,255,0.06), 0 0 24px rgba(124,60,255,0.05)',
          }}
        />
        ...
        {!reduced && (
          <>
            ...
            >
              <span
                className="absolute top-0 left-1/2 size-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary-bright"
            ...
            >
              <span
                className="absolute top-1/2 left-0 size-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-info"
        ...
        >
          <div className="relative flex flex-col items-center">
            ...
            >
              <div
                className="absolute inset-x-2 top-4 bottom-0 rounded-[50%] transition-colors duration-500"
                style={{
              ...
              />
              <div
                className="absolute inset-x-2 top-1 bottom-4 rounded-[50%] border transition-colors duration-500"
                style={{
              ...
              />
              <div
                className="absolute inset-x-9 top-3.5 bottom-6 rounded-[50%] border transition-colors duration-500"
                style={{ borderColor: `color-mix(in srgb, ${center.color} 32%, transparent)` }}
              />
              <div
                className="absolute -inset-x-4 top-6 -bottom-2 rounded-[50%] transition-colors duration-500"
                style={{
            ...
            >
              <span
                aria-hidden="true"
                className="pointer-events-none absolute top-1/2 left-1/2 size-20 -translate-x-1/2 -translate-y-[60%] rounded-full"
                style={{
              ...
              <span className="relative text-sm font-bold drop-shadow-[0_1px_3px_rgba(0,0,0,0.6)]">{center.name}</span>
              <span className="relative text-sm font-extrabold tabular-nums text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.6)]">
                {formatCurrency(center.amount, currency)}
              </span>
              <span className="relative text-xs font-bold drop-shadow-[0_1px_3px_rgba(0,0,0,0.5)]" style={{ color: center.color }}>
                {center.percent}%
              </span>
            </motion.button>
          </div>
          ...
          return (
            ...
            >
              <motion.button
                ...
                style={{ width: SATELLITE_SIZE, height: SATELLITE_SIZE, ...orbStyle(cat.color, false) }}
                whileHover={{ scale: 1.12 }}
                whileTap={{ scale: 0.9 }}
              >
                <span style={{ color: '#ffffff', filter: `drop-shadow(0 0 6px ${cat.color})` }}>
              ...
              <div
                ...
                style={reduced ? { opacity: 0.55 + slot.depth * 0.45 } : { opacity: 0 }}
              >
                <p className="max-w-24 truncate text-center text-xs font-bold text-foreground drop-shadow-[0_1px_3px_rgba(0,0,0,0.7)]">
                  {cat.name}
                </p>
                <p className="text-xs font-bold tabular-nums text-foreground/85 drop-shadow-[0_1px_3px_rgba(0,0,0,0.7)]">
                  {formatCurrency(cat.amount, currency)}
                </p>
                <p className="text-[11px] font-bold drop-shadow-[0_1px_3px_rgba(0,0,0,0.6)]" style={{ color: cat.color }}>
                  {cat.percent}%
                </p>
              </div>
            </div>
