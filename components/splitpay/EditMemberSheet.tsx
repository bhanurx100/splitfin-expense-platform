...
/**
 * components/splitpay/EditMemberSheet.tsx
 *
 * Modal sheet for editing an existing member.
 * Reuses the same field set as AddMemberSheet but pre-fills from the existing member.
 * Calls useGroupStore.updateMember on save — settlement recalculates automatically.
 */
...
export function EditMemberSheet({ groupId, member, onClose }: Props) {
  ...
  const handleSave = useCallback(() => {
    ...
    onClose();
  }, [canSave, updateMember, groupId, member.id, name, phone, upiId, color, onClose]);
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/5">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[15px] font-bold text-slate-900">Edit Member</h3>
              <p className="text-[12px] text-slate-400">Changes reflect in all expenses</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-4 p-5">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] text-slate-400">
              Default UPI: <span className="font-mono text-slate-600">{(phone || member.phone)}@upi</span>
            </p>
          </div>
