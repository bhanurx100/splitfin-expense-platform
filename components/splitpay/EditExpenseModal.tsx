...
/**
 * components/splitpay/EditExpenseModal.tsx
 *
 * Modal for editing an existing expense.
 * Reuses useExpenseForm with initialExpense to pre-fill all fields.
 * Calls useGroupStore.updateExpense on save.
 */
...
export function EditExpenseModal({ group, expense, onClose }: Props) {
  ...
  const {
    ...
    setAllParticipants, setParticipantValue,
  ...
  const handleSave = useCallback(() => {
    ...
    onClose();
  }, [validation, updateExpense, group.id, expense.id, form, grandTotal, onClose]);
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/5">
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4">
          <div>
            <h3 className="text-[15px] font-bold text-slate-900">Edit Expense</h3>
            <p className="text-[12px] text-slate-400">Changes recalculate settlements instantly</p>
          </div>
          <button
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <div>
            ...
            {showTaxTip && (
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                ] as const).map(({ label, field, val, presets }) => (
                  <div key={field}>
            ...
            {form.splitMethod !== "equal" && form.participants.length > 0 && (
              <div className="mt-3 space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                  {form.splitMethod === "exact"   && "Amount each person owes (₹)"}
                  {form.splitMethod === "percent"  && "Percentage each person pays (%)"}
                  {form.splitMethod === "shares"   && "Relative weight per person"}
                </p>
                ...
                {form.splitMethod === "percent" && (
                  <div className={cn(
                    ...
                    Math.abs(form.participants.reduce((s, p) => s + (p.value ?? 0), 0) - 100) < 0.5
                      ? "bg-emerald-50 text-emerald-700"
                      : "bg-amber-50 text-amber-700"
                  )}>
                    <span>Total</span>
                    <span>{form.participants.reduce((s, p) => s + (p.value ?? 0), 0).toFixed(1)}%</span>
                  </div>
                )}
                {form.splitMethod === "exact" && grandTotal > 0 && (
                  (() => {
                    const assigned = form.participants.reduce((s, p) => s + (p.value ?? 0), 0);
                    const diff = grandTotal - assigned;
                    return Math.abs(diff) > 0.01 ? (
                      <div className={cn(
                        "flex items-center justify-between rounded-lg px-2.5 py-1.5 text-[11px] font-semibold",
                        diff > 0 ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-600"
                      )}>
                      ...
                      <div className="flex items-center justify-between rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[11px] font-semibold text-emerald-700">
                        <span>All assigned</span><span>✓</span>
                      </div>
                    );
                  })()
                )}
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-[13px] text-slate-900 outline-none placeholder:text-slate-400 focus:border-blue-400 focus:bg-white transition"
            />
          </div>

          {/* Validation error */}
          {!validation.valid && form.amount > 0 && form.participants.length > 0 && (
